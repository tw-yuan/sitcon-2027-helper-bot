...
- **DR-4** 【硬性】**回覆給使用者**的 Drive 資訊僅限：檔名、自範圍資料夾起算的完整路徑、Drive URL、（若可得）檔案類型，外加 bot 自己寫的一句相關性說明。任何情況下不得把檔案內容、內文片段、縮圖轉述／摘要／引用給使用者；使用者要看內容請自行點連結（其 Drive 權限決定能否開啟）。`drive_search` 的結果型別在程式層即只承載 metadata。
