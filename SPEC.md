...
2. Google 共用雲端硬碟的檔案搜尋（僅回傳 metadata，絕不回傳內容）。
...
- **DR-3** 搜尋比對：檔名／資料夾名關鍵字為主，輔以 Drive API 全文索引（`fullText contains`）提升召回；全文索引僅作為伺服器端過濾條件，**bot 本身不下載、不讀取任何檔案內容**。
- **DR-4** 【硬性】搜尋結果僅回傳：檔名、自範圍資料夾起算的完整路徑、Drive URL、（若可得）檔案類型。任何情況下不回傳檔案內容、縮圖、內文片段；此限制在程式層強制（回應組裝僅接受 metadata 欄位）。
- **DR-5** 結果預設至多 10 筆並註明是否還有更多；使用者可要求下一批。
- **DR-6** (私) 開頭之資料夾與檔案照常列於結果（僅 metadata）；實際能否開啟由使用者本人的 Drive 權限決定。
...
- **NFR-5（安全—程式層防線）** GL-10、GL-16、DR-4、RO-2、HM-16 等硬性限制必須在程式層（工具參數驗證／API 封裝）強制，不得僅依賴 prompt 指示；LLM 產生的工具呼叫參數一律經白名單／schema 驗證後才執行。
- **NFR-6（安全—prompt injection）** 來自外部系統的內容（卡片描述、留言、筆記內文、檔名）注入 LLM 時一律標示為資料而非指令；其中出現的指示不得改變 bot 行為。以 NFR-5 為最終防線。
