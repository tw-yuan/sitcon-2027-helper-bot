...
- **GL-28** 建卡可帶 `link_to_iid`：建立後把新卡連進該卡的 Linked items（批次開卡以同一張母卡追蹤）；連結失敗不影響已建立的卡，需回報「卡已建立、連結失敗」。【2026-08-15 修訂】批次開卡（同一事項開給多組）**一律**先建母卡（標題「[全體] 事項 進度追蹤」；使用者指定既有卡則沿用）再逐張掛上，不待使用者要求。
